
import { Book, GraduationCap } from "lucide-react";

export const HubHero = () => {
  return (
    <div className="relative bg-[#0A0F1D] py-20">
      <div className="absolute inset-0 opacity-5 bg-[url('data:image/svg+xml;base64,')]" />
      
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex items-center justify-center mb-6">
          <div className="p-3 bg-[#F97316]/10 border border-[#F97316]/20 rounded-lg">
            <Book className="h-10 w-10 text-[#F97316]" />
          </div>
        </div>
        
        <h1 className="text-4xl md:text-5xl font-bold text-white text-center mb-4">
          Knowledge<span className="text-[#F97316]">Hub</span>
        </h1>
        
        <p className="text-lg text-mine-silver max-w-2xl mx-auto text-center mb-8">
          Expand your understanding of blockchain technology, AI infrastructure, and GPU mining through our curated learning pathways, resources, and community guides.
        </p>
        
        <div className="flex flex-wrap justify-center gap-4 max-w-3xl mx-auto">
          <div className="bg-[#0F172A] border border-[#F97316]/20 rounded-lg p-4 flex items-center gap-3 flex-1 min-w-[250px]">
            <div className="p-2 rounded-full bg-[#F97316]/10">
              <GraduationCap className="h-6 w-6 text-[#F97316]" />
            </div>
            <div>
              <h3 className="font-semibold text-white">Learning Pathways</h3>
              <p className="text-sm text-mine-silver">Structured courses from basics to advanced</p>
            </div>
          </div>
          
          <div className="bg-[#0F172A] border border-[#0EA5E9]/20 rounded-lg p-4 flex items-center gap-3 flex-1 min-w-[250px]">
            <div className="p-2 rounded-full bg-[#0EA5E9]/10">
              <Book className="h-6 w-6 text-[#0EA5E9]" />
            </div>
            <div>
              <h3 className="font-semibold text-white">Resource Library</h3>
              <p className="text-sm text-mine-silver">Whitepapers, research, and documentation</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
